...
export async function criarUsuarioAction(
  ...
  const role = String(formData.get("role") ?? "comprador") as "comprador" | "aprovador" | "estoquista" | "motorista";
  ...
  if (!["comprador", "aprovador", "estoquista", "motorista"].includes(role)) return { error: "Role inválido." };
...
export async function alterarRoleAction(profileId: string, novoRole: "comprador" | "aprovador" | "estoquista" | "motorista"): Promise<{ error?: string }> {
  const guard = await assertAprovador();
  if (!guard.ok) return { error: guard.error };
