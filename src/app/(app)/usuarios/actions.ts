...
export async function criarUsuarioAction(
  ...
  const role = String(formData.get("role") ?? "comprador") as "comprador" | "aprovador" | "estoquista" | "motorista" | "vendas" | "financeiro" | "gestor_producao";
  ...
  if (!["comprador", "aprovador", "estoquista", "motorista", "vendas", "financeiro", "gestor_producao"].includes(role)) {
...
export async function alterarRoleAction(profileId: string, novoRole: "comprador" | "aprovador" | "estoquista" | "motorista" | "vendas" | "financeiro" | "gestor_producao"): Promise<{ error?: string }> {
  const guard = await assertAprovador();
  if (!guard.ok) return { error: guard.error };
